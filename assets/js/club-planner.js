(function(root, factory){
  ...
  function init(){
    ...
    const DRAFT_KEY='leagueClubPlannerDraftV1';
    function saveDraft(){try{const d=readForm(form);d.step=current;localStorage.setItem(DRAFT_KEY,JSON.stringify(d));}catch(e){}}
    function restoreDraft(){let d=null;try{d=JSON.parse(localStorage.getItem(DRAFT_KEY)||'null')}catch(e){}if(!d)return null;const set=(id,v)=>{const el=document.getElementById(id);if(el&&v!=null)el.value=v};set('plan-team-size',d.teamSize);set('plan-grade',d.grade);set('plan-experience',d.experience);set('plan-adults',d.adults);set('plan-client',d.client);set('plan-tech',d.tech);set('plan-family',d.family);set('plan-access',d.access);form.querySelectorAll('input[name="plan-format-choice"]').forEach(i=>i.checked=i.value===d.format);form.querySelectorAll('input[name="plan-lesson-pattern"]').forEach(i=>i.checked=i.value===d.lessonPattern);form.querySelectorAll('input[name="plan-support"]').forEach(i=>i.checked=(d.supports||[]).includes(i.value));return Number(d.step)||1;}
    ...
    form.querySelectorAll('.format-choice-card input').forEach(input=>input.addEventListener('change',()=>{const group=input.name;form.querySelectorAll(`input[name="${group}"]`).forEach(i=>i.closest('.format-choice-card')?.classList.toggle('is-selected',i.checked));saveDraft()}));
    form.addEventListener('input',saveDraft);form.addEventListener('change',saveDraft);
    const restoredStep=restoreDraft();form.querySelectorAll('.format-choice-card input').forEach(i=>i.closest('.format-choice-card')?.classList.toggle('is-selected',i.checked));
    show(restoredStep||1,false);
